        ...
        if (spellIndex==0) { //Q
          ...
          for (int i = 0; (i < getEnemiesSize() && (!eCast)); i++){
            if (getEnemy(i).contains(getMouseX(), getMouseY()) && (Math.sqrt(Math.pow(getEnemy(i).getX()-getX(),2) + Math.pow(getEnemy(i).getY()-getY(),2)) < E_RANGE)){
              addStatus(new TimeMageE(getEnemy(i).getX(), getEnemy(i).getY()));//REE NOTE APPLIES TO ANYONE
              eCast = true;
            }
          }
          for (int i = 0; (i < getAlliesSize() && (!eCast)); i++){
            if (getAlly(i).contains(getMouseX(), getMouseY())){
              addStatus(new TimeMageE(getAlly(i).getX(), getAlly(i).getY()));//REE NOTE APPLIES TO ANYONE
              eCast = true;
            }
          }
          if (!eCast){
